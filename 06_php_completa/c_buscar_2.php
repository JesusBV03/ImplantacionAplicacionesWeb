...
<header>
    ...
    <main>
        ...
        $query = "select * from pisos where codigo_piso='$id_piso' and comprado = 0 or usuario_id='$id' and comprado = 0 or precio<='$pvp'  and comprado = 0 or metros<='$metros' and comprado = 0 or zona='$zona' and comprado = 0";
        ...
        if ($nfilas >= 1) {
            echo "<fieldset>";
            echo "<legend>Listar Usuario</legend>";
            echo "<form action='c_buscar_3.php' method='get' enctype='multipart/form-data'>";
            ...
            echo "<tr class='borde'>
                ...
                <td class='borde'>Comprar Vivienda</td>
            </tr>";

            for ($i = 0; $i < $nfilas; $i++) {
                $row = mysqli_fetch_assoc($listar);
                $_SESSION['precio_final_compra']=$row['precio'];
            ...
            <td class='borde'>" . $row['usuario_id'] . "</td><td class='borde'><input type='radio' name='compra' value='".$row['codigo_piso']."'></td></tr>";
            }
            echo "</table>";
            echo "</fieldset>";
        } else {
            echo "<fieldset>";
            echo "<legend>Listar Usuario</legend>";
            echo "<span  class='add_2'>No hay pisos en la base de datos</span>";
            echo "</fieldset>";
        }
        
        echo "<input class='margin_2' type='submit' name='comprar' value='Comprar'>";
        echo "</form>";
